...
from clinical_trials_client import get_clinical_trials_info
...
async def handle_fun_call(fun_name: str, params: str):
    ...
    elif fun_name == 'get_clinical_trials_info':
        param_dict = json.loads(params)
        nct_ids = param_dict['nctids']
        retval = json.dumps(await get_clinical_trials_info(nct_ids))
    else:
